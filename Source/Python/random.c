...
/* Amiga compatibility: define FD_CLOEXEC if not available */
#ifdef _AMIGA
#ifndef FD_CLOEXEC
#define FD_CLOEXEC 1
#endif
...
#if !defined(MS_WINDOWS) && !defined(__VMS) && !defined(_AMIGA)
...
#endif /* !defined(MS_WINDOWS) && !defined(__VMS) && !defined(_AMIGA) */

#ifdef _AMIGA
/* Amiga-specific random number generation */
static int
amiga_urandom(unsigned char *buffer, Py_ssize_t size, int raise)
{
    Py_ssize_t i;
    
    /* Use a simple but adequate random number generator for Amiga */
    /* In a production environment, you might want to use Amiga's random.library */
    for (i = 0; i < size; i++) {
...
#elif defined(_AMIGA)
    return amiga_urandom((unsigned char *)buffer, size, 1);
#else
# ifdef __VMS
...
#elif defined(_AMIGA)
        (void)amiga_urandom((unsigned char *)secret, secret_size, 0);
#elif defined(PY_GETENTROPY)
